...
def display_random_image (images, label, show_box = True ,image_path = 'local/data/imgs'):
    """
...
def display_random_image_Pre (Images,Images_Pre, show_box = True,image_path = 'local/data/imgs'):
    """
        Display a random image
    """
    ...
    if show_box == True:
        ...
        plt.figure(figsize=(15,8))
        plt.imshow(img)
        plt.title('Image #{} : Formato : {} '.format(index_img,Images.iloc[index_img][6]))
        plt.plot((xmin,xmax),(ymin,ymin),"r")
        plt.plot((xmin,xmax),(ymax,ymax),"r")
        plt.plot((xmin,xmin),(ymax,ymin),"r")
        plt.plot((xmax,xmax),(ymax,ymin),"r")
